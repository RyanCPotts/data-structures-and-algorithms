...
const longestString = (arr) => {
  // Initialize variables to store the index and length of the longest string
  let longestIndex = 0;
  let maxLength = 0;

  // Iterate over each string in the array
  arr.forEach((string, index) => {
    // If the current string is longer than the previous longest string, update the variables
  ...
  // Return the index position of the longest string
  return longestIndex;
};
...
const firstLetters = (arr) => {
  // Use map to extract the first letter of each string
...
const findHappiness = (arr) => {
  // Use filter to return only the strings that contain ":)"
...
const standardizePhoneNumbers = (arr) => {
  // Use map to remove non-digit characters from each phone number string
...
const onlyOddChars = (str) => {
  ...
  for (let i = 1; i < str.length; i += 2) {
    // Add the character at the odd index to the result string
    result += str[i];
  }

  // Return the result string containing only the odd-index characters
  return result;
};
...
const allHappy = (arr) => {
  // Use every to check if all strings contain ":)"
